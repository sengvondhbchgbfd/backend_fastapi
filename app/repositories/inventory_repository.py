...
class ProductRepository:
    ...
    async def get_all(
        self, company_id: int, category_id: int | None = None
    ) -> list[Product]:
        query = (
    ...
    async def update_stock(self, product: Product, qty_change: int) -> Product:
        ...
        await self.db.commit(); await self.db.refresh(product)
        return product
...
class StockMovementRepository:
    ...
    async def get_all(
        self, company_id: int, product_id: int | None = None
    ) -> list[StockMovement]:
        query = (
    ...
    async def get_by_id(
        self, movement_id: int, company_id: int
    ) -> StockMovement | None:
        r = await self.db.execute(
    ...
    async def get_balance(self, product_id: int, company_id: int) -> int:
        result = await self.db.execute(
            select(
                func.coalesce(func.sum(StockMovement.qty_in), 0) -
                func.coalesce(func.sum(StockMovement.qty_out), 0)
            ).where(
    ...
    async def delete(self, obj: StockMovement) -> None:
        await self.db.delete(obj); await self.db.commit()
...
class InvoiceRepository:
    ...
    async def get_all(
        self, company_id: int, customer_id: int | None = None
    ) -> list[Invoice]:
        query = (
