...
async def scan_authenticate(
    ...
    # ✅ require_login — must be logged in to re-authenticate
    ...
    return await scan_auth_service(
        ...
        # ✅ staff_id from JWT not request body
        staff_id = staff_id,
        db       = db,
    )
...
async def scan_check_in(
    ...
    """
    # ✅ company_id comes from scan_token decoded inside service
    # scan_token was issued with company_id embedded
    return await service.check_in(
        scan_token      = data.scan_token,
        office_qr_token = data.office_qr_token,
        latitude        = data.latitude,
        longitude       = data.longitude,
        company_id      = data.company_id,   # ✅ pass from request
    )
